...
public class Slider_Poke : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isFollowing)
        {
            Vector3 localTargetPosition = targetTransform.InverseTransformPoint(pokeAttachTransform.position + offset);
            ...
            targetTransform.position = targetTransform.TransformPoint(clampedLocal);
        }
        else if (resetPosition)
        {
            targetTransform.position = Vector3.Lerp(targetTransform.position, initialPosition, resetSpeed * Time.deltaTime);
        }
    ...
    public void Follow(BaseInteractionEventArgs hover)
    {
        if (hover.interactorObject is UnityEngine.XR.Interaction.Toolkit.Interactors.XRPokeInteractor)
        {
            ...
            offset = targetTransform.position - pokeAttachTransform.position;
            isFrozen = false;
        }
    }
