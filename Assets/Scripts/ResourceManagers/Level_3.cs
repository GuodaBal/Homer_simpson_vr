...
public class Level_3 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!game_over_triggered && level_duration > 0)
        {
            ...
            electricity.SetIncreaseSpeed((coal_in_reactor.GetCurrentValue() * 3.0f - electricity_consumption.GetCurrentValue() * 0.5f - electricity_demand.GetCurrentValue() * 0.7f) / speed);
            temperature.SetIncreaseSpeed((coal_in_reactor.GetCurrentValue() * 2.5f - cooling_power.GetCurrentValue()) / speed);
